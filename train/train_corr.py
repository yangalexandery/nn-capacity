...
from data_corr import MiniPlace
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corr', default=0, type = int)
    ...
    for split in ['train', 'test']:
        if split == 'train':
            data[split] = MiniPlace(data_path = os.path.join(args.data_path, args.synset), split = split, num_corrupted=args.corr)
        else:
            data[split] = MiniPlace(data_path = os.path.join(args.data_path, args.synset), split = split, num_corrupted=args.corr)
